...
from .fusion_adapter import Fusion_adapter
...
class Cat_Conv(nn.Module):
    ...
    def __init__(self, c, layer_idx, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(Cat_Conv, self).__init__()
        self.conv = Conv(2*c, c, k, s, p, g, act)

    def forward(self, rgb_x, lwir_x):
        x = torch.cat([rgb_x, lwir_x], 1)
        x = self.conv(x)
        return x, None, None
...
class ffno(nn.Module):
    def __init__(self, feat_dim=128, FEN=10):
        ...
        self.imnet = MLP(feat_dim*4 + 42, out_dim=feat_dim+1, hidden_list=self.mlp_dim)
        # Concat
...
class FFNO(nn.Module):
    def __init__(self, c, layer_idx):
        ...
        self.add_module(f'lwir_deformable_layer_{layer_idx}',
                        ModulatedDeformConv2dPack(c, c, kernel_size=3, padding=1))
        self.add_module(f'ffno_fuse_layer_{layer_idx}',ffno(c))

        self.add_module(f'dsc_layer_{layer_idx}', Conv(c, c, 1, g=c, act=nn.ReLU()))
        self.add_module(f'se_layer_{layer_idx}', SELayer(c))

        self.add_module(f'mask_layer_{layer_idx}',
                        nn.Sequential(
                            *[Conv(c, 64, 3, p=1, act=nn.ReLU()),
                              Conv(64, 64, 3, p=1, act=nn.ReLU()),
                              Conv(64, 1, act=False)]))
        self.add_module(f'mlp_layer_{layer_idx}',
                        nn.Sequential(*[nn.Conv2d(c, c//16, 1, bias=False),
                                        nn.ReLU(inplace=True),
                                        nn.Conv2d(c//16, c, 1, bias=False)]))
        self.add_module(f'spatial_layer_{layer_idx}', SpatialGate())

    def forward(self, rgb_x, lwir_x):
        tmp_rx = getattr(self, f'rgb_deformable_layer_{self.layer_idx}')(rgb_x)
        tmp_lx = getattr(self, f'lwir_deformable_layer_{self.layer_idx}')(lwir_x)
        tmp_fused_res = getattr(self, f'ffno_fuse_layer_{self.layer_idx}')(
            tmp_rx, tmp_lx)
        tmp_fused_res = tmp_fused_res.squeeze(2)
        pred_mask_logits, s_logits = [], []

        tmp_fused_res = getattr(self, f'dsc_layer_{self.layer_idx}')(tmp_fused_res)
        tmp_fused_res = getattr(self, f'se_layer_{self.layer_idx}')(tmp_fused_res)

        pred_mask_logits = getattr(self, f'mask_layer_{self.layer_idx}')(tmp_fused_res)
        v = self.cos(self.sigmoid(pred_mask_logits).flatten(2), tmp_fused_res.flatten(2), 2).unsqueeze(-1).unsqueeze(-1)
        s_logits = getattr(self, f'mlp_layer_{self.layer_idx}')(v)
        tmp_fused_res = self.sigmoid(s_logits) * tmp_fused_res
        tmp_fused_res = getattr(self, f'spatial_layer_{self.layer_idx}')(tmp_fused_res)

        return tmp_fused_res, pred_mask_logits, s_logits

def resize_sam_positional_embeddings(state_dict, new_hw=40):
    import torch.nn.functional as F

    if 'image_encoder.pos_embed' in state_dict:
        old_pos_embed = state_dict['image_encoder.pos_embed']  # [1, H_old, W_old, C]
        C = old_pos_embed.shape[-1]
        old_hw = int(old_pos_embed.shape[1] ** 0.5)
        old_pos_embed = old_pos_embed.permute(0, 3, 1, 2)  # [1, C, H, W]
        resized = F.interpolate(old_pos_embed, size=(new_hw, new_hw), mode='bicubic', align_corners=False)
        state_dict['image_encoder.pos_embed'] = resized.permute(0, 2, 3, 1)  # [1, H_new, W_new, C]

    def resize_rel_pos(rel_pos_key):
        if rel_pos_key in state_dict:
            old_rel_pos = state_dict[rel_pos_key]  # [2*old_len-1, C]
            old_len = (old_rel_pos.shape[0] + 1) // 2
            new_len = new_hw
            new_size = 2 * new_len - 1
            new_rel_pos = F.interpolate(
                old_rel_pos.unsqueeze(0).permute(0, 2, 1),
                size=new_size,
                mode='linear',
                align_corners=False
            ).permute(0, 2, 1).squeeze(0)
            state_dict[rel_pos_key] = new_rel_pos

    for layer in [2, 5, 8, 11]:
        resize_rel_pos(f'image_encoder.blocks.{layer}.attn.rel_pos_h')
        resize_rel_pos(f'image_encoder.blocks.{layer}.attn.rel_pos_w')

    return state_dict

class FFNO_sam(nn.Module):
    def __init__(self, c, layer_idx):
        super(FFNO_sam, self).__init__()
        self.layer_idx = layer_idx
        self.cos = torch.nn.functional.cosine_similarity
        self.sigmoid = torch.nn.Sigmoid()
        self.add_module(f'rgb_deformable_layer_{layer_idx}',
        ...
        self.add_module(f'lwir_deformable_layer_{layer_idx}',
                        ModulatedDeformConv2dPack(c, c, kernel_size=3, padding=1))
        self.add_module(f'ffno_fuse_layer_{layer_idx}',ffno_sam(c))
        ...
        self.add_module(f'mlp_layer_{layer_idx}',
                        nn.Sequential(*[nn.Conv2d(c, c//16, 1, bias=False),
                                        ...
                                        nn.Conv2d(c//16, c, 1, bias=False)]))
        self.add_module(f'spatial_layer_{layer_idx}', SpatialGate())
        cfg = '/home/MFOD/Methods/TFDet/yolov5-master/models/fuse-sam-vit-b.yaml'
        ...
        # Load SAM
        self.samadapter = sam_model.make(self.yaml['model'])
        sam_checkpoint = torch.load(self.yaml['sam_checkpoint'])
        sam_checkpoint = resize_sam_positional_embeddings(sam_checkpoint, new_hw=40)
        self.samadapter.load_state_dict(sam_checkpoint, strict=False)

        # Fine-tune最后两个block，其余冻结
        for name, para in self.samadapter.named_parameters():
    ...
    def forward(self, rgb_x, lwir_x, init_rgb, init_lwir):
        self.origin_h, self.origin_w = init_rgb.shape[2], init_rgb.shape[3]
        with torch.no_grad():
            sam_rgb = self.samadapter(init_rgb)
            sam_lwir = self.samadapter(init_lwir)

        tmp_rx = getattr(self, f'rgb_deformable_layer_{self.layer_idx}')(rgb_x)
        tmp_lx = getattr(self, f'lwir_deformable_layer_{self.layer_idx}')(lwir_x)
        tmp_fused_res = getattr(self, f'ffno_fuse_layer_{self.layer_idx}')(
            tmp_rx, tmp_lx, sam_rgb, sam_lwir)
