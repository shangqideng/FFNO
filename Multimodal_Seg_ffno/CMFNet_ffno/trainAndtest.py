...
FOLDER = "/home/MFSeg/ISPRS_dataset/" # Replace with your "/path/to/the/ISPRS/dataset/folder/"
...
def train(net, optimizer, epochs, scheduler=None, weights=WEIGHTS, save_epoch=2):
    ...
    for e in range(1, epochs + 1):
        ...
        os.makedirs('./results/Vaihingen/CMFNet_ffno/', exist_ok=True)
        if e % save_epoch == 0:
            # We validate with the largest possible stride for faster computing
            acc = test(net, test_ids, all=False, stride=32)
            if acc > acc_best:
                torch.save(net.state_dict(), './results/Vaihingen/CMFNet_ffno/segnet256_epoch{}_{}'.format(e, acc))
                acc_best = acc
...
#####   test   ####
net.load_state_dict(torch.load('./results/Vaihingen/CMFNet_ffno/segnet256_epoch28_91.3956671338284'))
...
for p, id_ in zip(all_preds, test_ids):
   ...
   io.imsave('./results/Vaihingen/CMFNet_ffno/inference_tile_{}.png'.format(id_), img)
