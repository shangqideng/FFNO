...
from UNetFormer_MMSAM import UNetFormer_ffno as MFNet
try:
        ...
        if e % save_epoch == 0:
            ...
            if MIoU > MIoU_best:
                if DATASET == 'Vaihingen':
                    torch.save(net.state_dict(), './results/Vaihingen/MFNet_ffno1/{}_epoch{}_{}'.format(MODEL, e, MIoU))
...
if MODE == 'Train':
    os.makedirs('./results/Vaihingen/MFNet_ffno1/', exist_ok=True)
    train(net, optimizer, epochs, scheduler, weights=WEIGHTS, save_epoch=save_epoch)

elif MODE == 'Test':
    if DATASET == 'Vaihingen':
        net.load_state_dict(torch.load('./results/Vaihingen/MFNet_ffno/UNetformer_epoch28_0.8456315011121621'), strict=False)
        net.eval()
        # from thop import profile, clever_format
        # import torch.nn as nn
        # from thop.vision.basic_hooks import count_convNd, zero_ops

        # def count_bn(module, input, output):
        #     input = input[0]
        #     flops = 2 * input.numel()
        ...
        # def add_flops_counter_hook(model):
        #     for m in model.modules():
        #         m.register_buffer('total_ops', torch.zeros(1))
        #         m.register_buffer('total_params', torch.zeros(1))

        # add_flops_counter_hook(net)
        # dummy_rgb = torch.randn(1, 3, 256, 256).cuda()
        # dummy_dsm = torch.randn(1, 256, 256).cuda()

        # with torch.no_grad():
        #     macs, params = profile(net, inputs=(dummy_rgb, dummy_dsm), custom_ops=custom_ops)
        ...
        for p, id_ in zip(all_preds, test_ids):
            ...
            # io.imsave('./results/Vaihingen/MFNet_ffno/inference_UNetFormer_{}_tile_{}.png'.format('huge', id_), img)

    elif DATASET == 'Potsdam':
        net.load_state_dict(torch.load('./results/Vaihingen/MFNet/UNetformer_epoch32_0.8401698029093371'), strict=False)
        net.eval()
        MIoU, all_preds, all_gts = test(net, test_ids, all=True, stride=32)
        print("MIoU: ", MIoU)
        for p, id_ in zip(all_preds, test_ids):
