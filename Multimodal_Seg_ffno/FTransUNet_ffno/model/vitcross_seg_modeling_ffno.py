...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, y):
        ...
        x1 = x.contiguous().view(b, 16, 16, c).permute(0, 3, 1, 2)     # [24, 768, 16, 16]
        y1 = y.contiguous().view(b, 16, 16, c).permute(0, 3, 1, 2)
        x = self.ffno(x1, y1)
        x = x.contiguous().permute(0, 2, 3, 1).view(b, -1, c)
        # x = x + y
