...
from model.vitcross_seg_modeling_ffno import VisionTransformer as ViT_seg
from model.vitcross_seg_modeling_ffno import CONFIGS as CONFIGS_ViT_seg
try:
...
config_vit.pretrained_path = "/home/MFSeg/methods/SSRS-main/imagenet21k+imagenet2012_R50+ViT-B_16.npz"
    ...
    for e in range(1, epochs + 1):
        ...
        for batch_idx, (data, dsm, target) in enumerate(train_loader):
            ...
            if iter_ % 500 == 0:
                ...
                net.train()
                if acc > acc_best:
                    torch.save(net.state_dict(), './results/Vaihingen/FtransUNet_ffno/segnet256_epoch{}_{}'.format(e, acc))
                    acc_best = acc
    print('acc_best: ', acc_best)

# #####   train   ####
# os.makedirs('./results/Vaihingen/FtransUNet_ffno/', exist_ok=True)
...
####   test   ####
net.load_state_dict(torch.load('./results/Vaihingen/FtransUNet/segnet256_epoch33_92.23114464996104'))
net.eval()
# from thop import profile
# from thop import clever_format

# # 模拟输入：RGB图像和DSM，大小为 [1, C, H, W]
# dummy_rgb = torch.randn(1, 3, 256, 256).cuda()
# dummy_dsm = torch.randn(1, 256, 256).cuda()

# # FLOPs 和参数统计
# macs, params = profile(net, inputs=(dummy_rgb, dummy_dsm))
...
for p, id_ in zip(all_gts, test_ids):
    ...
    io.imsave('./results/Vaihingen/FtransUNet/gts_tile{}.png'.format(id_), img)
