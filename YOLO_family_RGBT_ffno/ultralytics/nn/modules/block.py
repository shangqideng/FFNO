...
class FFNO(nn.Module):
    def __init__(self, c, c_out, layer_idx):
        ...
        self.add_module(f'rgb_deformable_layer_{layer_idx}',
                        ModulatedDeformConv2dPack(c, c_out*2, kernel_size=3, padding=1))
        self.add_module(f'lwir_deformable_layer_{layer_idx}',
                        ModulatedDeformConv2dPack(c, c_out*2, kernel_size=3, padding=1))
        self.add_module(f'inr_fuse_layer_{layer_idx}',INR_Fus_v4_3(c_out*2))
    ...
    def forward(self, x):
        ...
        tmp_fused_res = getattr(self, f'inr_fuse_layer_{self.layer_idx}')(
            tmp_rx, tmp_lx)
        
        tmp_fused_res = tmp_fused_res.squeeze(2)
        return tmp_fused_res
    
class FFNO_1(nn.Module):
    def __init__(self, c, c_out, layer_idx):
        super(FFNO_1, self).__init__()
        self.layer_idx = layer_idx
        self.cos = torch.nn.functional.cosine_similarity
        self.sigmoid = torch.nn.Sigmoid()
        self.add_module(f'rgb_deformable_layer_{layer_idx}',
                        nn.Sequential(
                nn.Conv2d(c // 2, c_out, kernel_size=3, padding=1),
        ...
        self.add_module(f'inr_fuse_layer_{layer_idx}',INR_Fus_v4_3(c_out))
        # # channel weight layer

    def forward(self, x):
        rgb_x, lwir_x = x[..., 0:3, :, :], x[..., 3:6, :, :]
        tmp_rx = getattr(self, f'rgb_deformable_layer_{self.layer_idx}')(rgb_x)
        tmp_lx = getattr(self, f'lwir_deformable_layer_{self.layer_idx}')(lwir_x)
        tmp_fused_res = getattr(self, f'inr_fuse_layer_{self.layer_idx}')(
            tmp_rx, tmp_lx)
